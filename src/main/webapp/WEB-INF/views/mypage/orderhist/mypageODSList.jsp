...
<form id="searchForm" name="searchForm" action="/p/mpa/selectOrdDlvCrst"
	method="get">
	<input type='hidden' name='searchType' id='searchType' value="2" /> <input
		type='hidden' name='ordStrtDt' id='ordStrtDt' value="" /> <input
		type='hidden' name='ordEndDt' id='ordEndDt' value="" /> <input
		type='hidden' name='selectTypeGbcd' id='selectTypeGbcd' value="" /> <input
		type='hidden' name='itemNm' id='itemNm' value="" /> <input
		type='hidden' name='listSize' id='listSize' value="" /> <input
		...
		<ul class="dotlist">
			<li>구매확정이 완료된 주문은 진행중인 주문에 포함되지 않으며, 진행상태에 따라 배송지 변경, 취소, 교환,
				반품신청이 가능합니다.</li>
			<li>가전, 가구 등 설치 상품은 물류 이동으로 인해 상품발송 후 설치 방문까지 2~3일 정도 기간이 추가될 수
				있습니다.</li>
		</ul>
		...
		<div class="guide-box">
			...
			<ul class="dotlist">
				...
				<li>결제수단을 무통장 입금이나 상품권으로 선택한 경우, 주문일로부터 <strong
					class="color-666">7일 이내 입금 및 접수확인이 되지 않으면 주문이 취소</strong>됩니다.
				</li>
