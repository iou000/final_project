...
$(document).ready(function () {
	$("#cancelconfirm").click(function() {
		...
		var discountPrice = $("input[name=disPrice]").val(); // 전체 주문 할인 금액
		...
		var returnPrice = pmtPrice * cancelCountProdPercentPrice; // 환불할 금액
		var returnPoint = resvDiscPrice * cancelCountProdPercentPrice; // 환불할 포인트
		var returnCpn = cpnDiscPrice * cancelCountProdPercentPrice; // 
		
		
		// 동적 tag생성
		var pmtAmount = Math.floor(oneProdPercentPrice * orderedCount * pmtPrice); // 해당 상품 총 결제 금액(현금 + 포인트)
		console.log('pmtAmount : ' + pmtAmount);
		$("#orgStlmAmt").html(AddComma(pmtAmount));
		if (resvDiscPrice != 0 || cpnDiscPrice != 0) {
			var discount = '${odDTO.prd_price * odDTO.prd_count}' - pmtAmount; // 해당 상품에 대해 할인받은 금액
			$("#orgDisc").show();
			$("#orgDisc").children('dd').find('strong').html(AddComma(discount));			
		}
		var ptPmtAmount = Math.floor(resvDiscPrice * oneProdPercentPrice * '${odDTO.prd_count}'); // 해당 상품에 대해 사용한 포인트
		console.log('ptPmtAmount : ' + ptPmtAmount);
		var pmtCash = pmtAmount - ptPmtAmount; // 해당 상품에 대해 지불한 금액(현금)
		console.log('pmtCash : ' + pmtCash);
		if ('${oDTO.pay_method}' == '카드') {
			$("#pchsMthd").children('dt').html("카드");
		} else if ('${oDTO.pay_method}' == '무통장입금') {
			$("#pchsMthd").children('dt').html("현금");
		}
		$("#pchsMthd").children('dd').find('strong').html(AddComma(pmtCash));

		if (ptPmtAmount != 0) {
			$("#pchsPoint").show();
			$("#pchsPoint").children('dd').find('strong').html(AddComma(ptPmtAmount));
		}
		$("#schdCnclAmt").html(AddComma(pmtAmount));
		$("#stlmOrdAmt").html(AddComma(pmtAmount));
		var lestPayment = '${odDTO.prd_price * odDTO.prd_count}' - discount - pmtAmount;
		$("#rmndStlmAmt").html(AddComma(lestPayment));
		$("#stlmRepyPossAmt").html(AddComma(lestPayment));

		// 환불금액 정보
		// 취소금액
		$("#dtlCnclAmt").html(AddComma(Math.floor(returnPrice)));
		// 원 주문금액
		var originPrice = '${odDTO.prd_price * odDTO.prd_count}';
		var originOnePrice = Math.floor(originPrice / '${odDTO.prd_count}');
		$("#dtlOrgOrdAmt").html(AddComma(Math.floor(originOnePrice * cancelCount)));
		// 쿠폰/할인
		if (resvDiscPrice != 0 || cpnDiscPrice != 0) {
			var discount = '${odDTO.prd_price * odDTO.prd_count}' - pmtAmount; // 해당 상품에 대해 할인받은 금액
			$("#dtlCpDcAmt").html('(-) ' + AddComma(Math.floor(returnPoint)));
		}
		...
		$("#stlmRtpAmt_0").html(AddComma(Math.floor(returnPrice)));
		
		if (ptPmtAmount != 0) { // 포인트 결제 내역이 있을 경우
			$("#rfndPt").show();
		...
		$('input[name=updtDis]').val(discountPrice - Math.floor(returnPoint) - Math.floor(returnCpn));
...
function insertOrdCncl() {
	...
	var datas = { "updtTotal":updtTotal, "updtDis":updtDis, "updtPmt":updtPmt,
					...
					"orderId":orderId, "oDetailId":${orderDetailNo} };
	console.table(datas);
	//온클릭시 호출
	$.ajax({
		type:"POST",  
	                                ...
	                                <ul>
	                                    ...
	                                    <li>
	                                        ...
	                                        <dl class="between" id="dtlAddIrgnDlvcAmtTr2">
	                                            ...
	                                            <input type="hidden" id="repyPossAmt" value="5500" />
	                                        </dl>
	                                    </li>
	                                </ul>
	                            </div>
	                                ...
	                                <ul>
	                                    ...
	                                    <li>
	                                        ...
	                                        <dl class="between" name="paywayTr" id="rfndPt" style="display: none">
	                                            ...
	                                            <dd><strong id="stlmRtpAmt_1" name="stlmRtpAmt">0</strong><span id="stlmRtpWon_1">원</span></dd>
	                                        </dl>
	                                    </li>
	                                </ul>
	                            </div>
